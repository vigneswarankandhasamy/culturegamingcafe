...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    $checkStmt = $conn->prepare(
        ...
        AND session_date = ? 
        AND (
            (session_from_time < ? AND session_to_time > ?) OR
            (session_from_time >= ? AND session_from_time < ?) OR
            (session_to_time > ? AND session_to_time <= ?) OR
            (session_from_time = ? AND session_to_time = ?)
        )"
    );
    $checkStmt->bind_param(
        ...
        $session, 
        $session_date, 
        $session_to_time, $session_from_time, 
        $session_from_time, $session_to_time, 
        $session_from_time, $session_to_time,
        $session_from_time, $session_to_time
    );
    $checkStmt->execute();
